...
const VerifyDelivery = () => {
      ...
      try {
        ...
        if (err.response) {
          if (err.response.status === 401) {
            ...
            setError("Failed to load delivery boy.");
          }
        } else {
  ...
  const verifyFarmer = async (farmerId) => {
    try {
      await axios.put(`/api/admin/verify-delivery/${farmerId}`, {}, { withCredentials: true });
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-2xl rounded-2xl overflow-hidden">
        ...
        <div className="bg-green-600 text-white p-6 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-2xl font-bold">Delivery Boy Verification Dashboard</h1>
          </div>
          ...
          {farmers.length === 0 ? (
            <div className="text-center py-10 bg-green-50 rounded-lg">
              ...
              <p className="text-xl text-green-800 font-semibold">
                No Delivery Boy waiting for approval at the moment
              </p>
              <p className="text-green-700 mt-2">
                All Delivery Boy are currently verified or no new applications exist
              </p>
            </div>
          ) : (
            <div>
              <div className="flex items-center mb-6 space-x-2">
                ...
                Delivery Boy Awaiting Verification
                </h2>
              </div>
                ...
                {farmers.map((farmer) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-green-700">{farmer.email}</p>
                    </div>
                    <button
                    ...
                    >
                      <CheckCircle size={20} />
                      <span>Approve Delivery Boy</span>
                    </button>
                  </div>
                ))}
              </div>
